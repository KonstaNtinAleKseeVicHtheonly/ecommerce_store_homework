...
from app.config import YOOKASSA_RETURN_URL, YOOKASSA_SECRET_KEY, YOOKASSA_SHOP_ID, YANDEX_IP_LIST
import ipaddress
...
async def create_yookassa_payment(
    ...
    '''асинхронное создание платежа в юкассе
    прнимает инфу о заказе, user_emai, описание товара
    '''
    project_logger.info(f"начало создание платежа по заказу для юкассы| SECRET_KEY = {YOOKASSA_SECRET_KEY}")
    ...
    # 4. Вспомогательная синхронная функция для создания платежа - асинхронно отправляет POST-запрос к API
    ...
    # 6. Извлечение URL для оплаты - ссылка для редиректа
    confirmation_url = getattr(payment.confirmation, "confirmation_url", None)
    project_logger.info("Платеж для юкасса успешно сформировани")
...
def is_ip_allowed(ip:str|None)->bool:
    '''проверяет валидность ip по которому запрос приходит'''
    if ip is None:
        return False
    try:
    ...
    for mask in YANDEX_IP_LIST:
        if "/" in mask:# если есть / значит это маска подсети
            if address in ipaddress.ip_network(mask, strict=False): # stricFalse - игнорирование строгих проверок
                return True
        else:
            if address == ipaddress.ip_address(mask): # проверка точечного ip адреса
                return True
    return False
    
def _extract_client_ip(request: Request) -> str | None:
    '''определение конкретного ip адреса с учетом работы NGINX
    без нее request.client.host вернет ip послденего прокси и вызовет ошибки при сверке ip'''
    forwarded_for = request.headers.get("x-forwarded-for")
    if forwarded_for:
        return forwarded_for.split(",")[0].strip()# за всеми прокси берем исходный Ip рельного клиента От которого запрос шел
